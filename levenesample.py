...
#entries we want to run levene on. fill this array out with the numbers of the samples you want to run.
...
for i in range(len(entryList)):
    arrayList.append(np.array([float(x) for x in df.iloc[entryList[i]][1:].transpose()])) #converting rows into arrays

statistic, pvalue = scipy.stats.levene(*arrayList)
